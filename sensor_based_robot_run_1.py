...
def range_smoother(range_info,min,max):
    ...
    for i in range(len(range_info)):
        ...
        if(range_info[i]==max):
            continue
        if(float(range_info[i])-0.1)<0:
            smooth_array.append(0)
        else:
            smooth_array.append(float(range_info[i])-0.1)
        # smooth_array.append(float(range_info[i]))
    #smooth_array = [float(ele) for ele in smooth_array]
    #smooth_array = list(map(float, smooth_array))
    return smooth_array
...
def callback(msg):
    ...
    global ranges,angles,line,scan
    scan = msg
    ranges = []
    angles = []
    
    
    ranges = range_smoother(scan.ranges,scan.range_min,scan.range_max)
    #ydata = R_max_set
    #ranges=float(ranges)
    #for i in len(ranges):
        #ranges[i]-=0.1
    # ranges = ranges-0.1
    #ranges = ranges - 0.1
    angles = np.linspace(scan.angle_min,scan.angle_max,num = len(ranges))
    #xdata = np.linspac
    #print(Gaps)
...
chosenGoals=np.array([[1.37,0.85],[1.68,0.57],[2,0]])
g=0
...
new_time=old_time
out_xr = open("out_xr_{}.txt".format(test_number), "a")
out_yr = open("out_yr_{}.txt".format(test_number), "a")
out_gx = open("out_gx_{}.txt".format(test_number), "a")
out_gy = open("out_gy_{}.txt".format(test_number), "a")
out_surf = open("out_surf_{}.txt".format(test_number), "a")
out_v = open("out_v_{}.txt".format(test_number), "a")
out_omega = open("out_omega_{}.txt".format(test_number), "a")
out_x = open("out_x_{}.txt".format(test_number), "a")
out_y = open("out_y_{}.txt".format(test_number), "a")
out_theta = open("out_theta_{}.txt".format(test_number), "a")
out_times = open("out_times_{}.txt".format(test_number), "a")
out_timens = open("out_timens_{}.txt".format(test_number), "a")
while not rospy.is_shutdown():    #loop to provide delay
    ...
    try:
        ...
        old_time=new_time
        out_times.write("%s ," % time_diff.secs)
        out_timens.write("%s ," % time_diff.nsecs)
        ...
        selected_goal=chosenGoals[g]
        print('selected_goal:',selected_goal)
        print('dis',((selected_goal[0]-x)**2+(selected_goal[1]-y)**2)**0.5)

        if ((selected_goal[0]-x)**2+(selected_goal[1]-y)**2)**0.5<0.1:
            g=g+1
            if g==3:
                break
        ...
        print("goal:",goal)
        out_gx.write("%s ," % goal[0])
        out_gy.write("%s ," % goal[1])
        ...
        # plt.show()
        SG= np.vstack((SG, [selected_goal[0], selected_goal[1]]))
        ...
        while not reached(X[:, -1], goal):      #loop to run to interim goal
            ...
            out_x.write("%s ," % x)
            out_y.write("%s ," % y)
            out_theta.write("%s ," % theta)
            X = np.column_stack((X,[x,y,X[2, -1]+diff]))
            # out_time.write("%s ," % time)
...
for i in range(len(SG)-1):
    plt.plot(SG[i,0], SG[i,1], 'o', color='red', linewidth=1.5)
plt.ylabel('Position Y')
plt.xlabel('Position X')
plt.title('Position')
plt.legend(['path', 'last', 'start', 'waypoint','selected midpoint'])
